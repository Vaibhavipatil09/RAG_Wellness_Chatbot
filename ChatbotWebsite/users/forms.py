...
# psychologist registration form (creates an unverified account)
class PsychologistRegistrationForm(FlaskForm):
    username = StringField(
        "Full Name", validators=[DataRequired(), Length(min=2, max=20)]
    )
    email = StringField("Email", validators=[DataRequired(), Email()])
    license_number = StringField(
        "License / Registration Number", validators=[DataRequired(), Length(min=3, max=50)]
    )
    specialty = StringField(
        "Specialty", validators=[DataRequired(), Length(max=100)]
    )
    password = PasswordField("Password", validators=[DataRequired(), Length(max=20)])
    confirm_password = PasswordField(
        "Confirm Password", validators=[DataRequired(), EqualTo("password")]
